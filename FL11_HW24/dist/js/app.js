...
function whoWin(userMove, botMove) {
  ...
  if (userMove != botMove) {
    if (userMove == rock && botMove == scissors) {
      game_wins++;
      move.innerHTML = `Round ${game_round},  Rock vs. Scissors, You’ve WON!`;
    } else if (userMove == rock && botMove == paper) {
      game_losts++;
      move.innerHTML = `Round ${game_round},  Rock vs. Paper, You’ve LOST!`;
    }

    if (userMove == scissors && botMove == paper) {
      game_wins++;
      move.innerHTML = `Round ${game_round},  Scissors vs. Paper, You’ve WON!`;
    } else if (userMove == scissors && botMove == rock) {
      game_losts++;
      move.innerHTML = `Round ${game_round},  Scissors vs. Rock, You’ve LOST!`;
    }

    if (userMove == paper && botMove == rock) {
      game_wins++;
      move.innerHTML = `Round ${game_round},  Paper vs. Rock, You’ve WON!`;
    } else if (userMove == paper && botMove == scissors) {
      game_losts++;
      move.innerHTML = `Round ${game_round},  Paper vs. Scissors, You’ve LOST!`;
    }
  } else {
    move.innerHTML = `Round ${game_round},  DROW!`;
  }

  if (game_round === 3) {
    resetGameBtn.classList.remove('hide');

    if (game_wins === game_losts) {
      result.innerHTML = "Game is finished. DROW!";
    }

    game_wins > game_losts ? result.innerHTML = `Game is finished. You've WON!` : result.innerHTML = `Game is finished. You've LOST!`;
  }
}
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "resetGameBtn", function() { return resetGameBtn; });
